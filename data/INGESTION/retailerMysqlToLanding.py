...
spark = SparkSession.builder.appName("RetailerMysqlTo").getOrCreate()

#GCS configurations
GCS_BUCKET = 'datalake-bkt-06042025'
LANDING_PATH = f"gs://{GCS_BUCKET}/landing/retailerDB/"
ATCHIEVE_PATH = f"gs://{GCS_BUCKET}/landing/retailerDB/archive/"
CONFIG_FILE_PATH = f'gs://{GCS_BUCKET}/config/retailer_config.csv'

#Bigquery configrations
BQ_PROJECT = 'sagar-720'
...
MYSQL_CONFIG = {
    "url":"jdbc:mysql://34.136.189.236:3306/retailerDB?useSSL=false&allowPublicKeyRetrieval=true",
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        query = f"(SELECT * FROM {table}) AS t" if load_type.lower() == "full load" else \
                f"(SELECT * FROM {table} WHERE updated_at > '{last_watermark}') AS t"
